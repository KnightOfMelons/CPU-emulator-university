# Сделано Ильёй с любовью и под музыку из игры Гарри Поттер и Философский камень ()
...
stack = []
MAX_STACK_SIZE = 15  # Максимальный размер стека добавил, вроде в ТЗ говорилось об ограничении в 7-15 элементов.
...
def execute_instruction(instruction):
    ...
    if mnemonic == "PUSH":
        if len(parts) != 2 or not parts[1].isdigit():
            print("\n=== Ошибка: неверная команда PUSH ===")
            return

        if len(stack) >= MAX_STACK_SIZE:
            print("\n=== Ошибка: стек переполнен, невозможно добавить новый элемент ===")
            return

        value = int(parts[1])
        stack.append(value)

    # Команда ADD для складывания двух верхних элементов стека
    elif mnemonic == "ADD":
        if len(stack) < 2:
            print("\n=== Ошибка: недостаточно элементов в стеке для выполнения операции 'ADD' ===")
            return
    ...
    # Команда DELETE для удаления последнего элемента стека
